    ...
    <header class="bg-white shadow-md z-20">
        <div class="container mx-auto px-2 py-3 flex items-center justify-between">
            ...
            <button @click="isMenuOpen = !isMenuOpen" class="sm:hidden text-gray-800 focus:outline-none">
                <UIcon :name="isMenuOpen ? 'i-mdi-close' : 'i-mdi-menu'" :color="isMenuOpen ? 'primary' : 'accent'"
                    class="text-3xl" />
            </button>

            <!-- Navigation Menu -->
            <nav class="max-sm:hidden">
                <ul class="flex flex-row items-center space-x-3 lg:space-x-6 space-y-0 px-0 py-0">
        ...
        <nav v-if="isMenuOpen" class="w-full pb-4 sm:hidden bg-muted shadow-md z-10 ease-in-out duration-300">
